...
class TrainerBase:
    ...
    def model_subspace_backward_and_update(self, loss, names=None):
        self.model_zero_grad(names)
        self.model_backward(loss)
        ...
        for name in names:
            for param in self._models[name].parameters():
                grad_lines.append(param.grad.clone().reshape(-1))
        grad_lines = torch.cat(grad_lines, 0)
        
        grad_lines = torch.mm(self.U, grad_lines.reshape(-1, 1))
        grad_proj = torch.mm(self.U.transpose(0, 1), grad_lines)
        idx = 0
        for name in names:
            for param in self._models[name].parameters():
                arr_shape = param.grad.shape
                size = arr_shape.numel()
                param.grad.data = grad_proj[idx:idx+size].reshape(arr_shape).clone()
                idx += size
